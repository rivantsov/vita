...
namespace Vita.Data.Linq.Translation {
    ...
    partial class ExpressionTranslator
    {
        ...
        public virtual TableExpression RegisterAssociation(TableExpression tableExpression, EntityMemberInfo refMember, TranslationContext context)
        {
            ...
            string joinID;
            var theseKeys = GetAssociationMembers(tableExpression, refMember, out otherKeys, out joinType, out joinID, context.DbModel);
        ...
        private IList<MemberInfo> GetAssociationMembers(TableExpression thisTableExpression, EntityMemberInfo memberInfo,
                             out IList<MemberInfo> otherKey, out TableJoinType joinType, out string joinID, DbModel dbModel) {
          if (memberInfo != null && memberInfo.Kind == Vita.Entities.Model.MemberKind.EntityRef) {
            // by default, join is inner
            joinType = TableJoinType.Inner;
            joinID = memberInfo.MemberName;
            var otherType = memberInfo.ReferenceInfo.ToKey.Entity.ClassInfo.Type;
            otherKey = memberInfo.ReferenceInfo.ToKey.ExpandedKeyMembers.Select(km => (MemberInfo)km.Member.ClrClassMemberInfo).ToList();
            var thisKey = memberInfo.ReferenceInfo.FromKey.ExpandedKeyMembers.Select(km => (MemberInfo)km.Member.ClrClassMemberInfo).ToList();
