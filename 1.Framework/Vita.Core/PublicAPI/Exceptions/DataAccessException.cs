...
namespace Vita.Entities {
  ...
  public class DataAccessException : Exception {
    ...
    public DataAccessException(Exception sqlException, 
                               IDbCommand command = null, string entityCommandName = null)   
      : base(sqlException.Message, sqlException) {
      Command = command; 
      EntityCommandName = entityCommandName;
      if (!string.IsNullOrWhiteSpace(EntityCommandName))
        this.AddValue("EntityCommandName", EntityCommandName);
    }
    //Used by Linq engine to throw linq translation failure
