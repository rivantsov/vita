...
namespace Vita.Web {
  ...
  /// <summary>HTTP message handler providing basic integration of VITA facilities with Web API stack. 
  ...
  /// </summary>
  public class WebCallContextHandler : DelegatingHandler, IWebCallNotificationService {
    ...
    #region WebCallInfo helper class, a container for Request, Response and WebContext
    internal class WebCallInfo {
      ...
      private IList<Cookie> GetIncomingCookies(string name) {
        return WebHelper.GetCookies(Request, name);
      }
      private IList<string> GetIncomingHeaderValues(string name) {
        return WebHelper.GetHeaderValues(Request, name);
      }
    }
    #endregion
