...
namespace Vita.Modules.Login {
  ...
  public static class LoginExtensions {
    ...
    internal static IPasswordHistory CreatePasswordHistoryEntry(this ILogin login) {
      var session = EntityHelper.GetSession(login);
      var hist = session.NewEntity<IPasswordHistory>();
      hist.Login = login;
      hist.WeakPasswordHash = login.WeakPasswordHash;
      // disable previous entries
      var now = session.Context.App.TimeService.UtcNow;
      var oldEntries = session.EntitySet<IPasswordHistory>().Where(h => h.Login == login && h.DisabledOn == null).ToList();
      foreach(var oldE in oldEntries)
        oldE.DisabledOn = now; 
      return hist;
    }
