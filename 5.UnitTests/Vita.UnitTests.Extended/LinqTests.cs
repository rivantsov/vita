 ...
 */ 
    ...
    public void TestLinqQueryCache() {
      ...
      var kidBooksQ = from b in books
                      ...
                      orderby b.Title
                      select b;
      var entQuery = kidBooksQ as EntityQuery;
    ...
    public void TestLinqSpecialQueries() {
      ...
      if (SetupHelper.ServerType != DbServerType.SqlCe) {
        ...
        //SQLCE has some specifics - it does not allow Text(memo) columns in output of DISTINCT clause; 
        // so doing easier query without book.Abstract column
        var qJoin = from b in books
                    join p in pubs on b.Publisher equals p
                    where p.Name == "MS Books"
                    select new { b.Id, b.Title, PubId = p.Id, p.Name }; //new { B = b, P = p }; //
        var joinDistinct = qJoin.Distinct().ToList();
        Assert.IsTrue(joinDistinct.Count > 0, "JoinDistinct query failed.");
        var joinCount = qJoin.Count();
        Assert.IsTrue(joinCount > 0, "JoinCount query failed.");
