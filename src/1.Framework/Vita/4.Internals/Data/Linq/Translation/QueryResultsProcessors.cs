...
namespace Vita.Data.Linq.Translation {
  ...
  public abstract class RowListProcessor {
    public abstract object ProcessRows(IEnumerable rows);

    public static RowListProcessor CreateFirstSingleLast(string methodName, Type rowType) {
      var processorType = typeof(FirstSingleLastProcessor<>).MakeGenericType(rowType);
      var processor = (RowListProcessor)Activator.CreateInstance(processorType, methodName);
      return processor;
    }

    public static RowListProcessor CreateGroupBy(Type keyType, Type rowType) {
      var processorType = typeof(GroupByProcessor<,>).MakeGenericType(keyType, rowType);
      var result = (RowListProcessor)Activator.CreateInstance(processorType);
      return result;
    }
  }

  #region First/Single/Last processor
  public class FirstSingleLastProcessor<TRow> : RowListProcessor {
  ...
  public class GroupByProcessor<TKey, TRow> : RowListProcessor {
    public override object ProcessRows(IEnumerable rows) {
  ...
  public static class ColumnReaderHelper {
    ...
    public static Expression GetColumnValueReader(ParameterExpression dataRecordParameter, int valueIndex, Type outputType, Func<object, object> conv) {
      _readColumnMethod = _readColumnMethod ?? typeof(ColumnReaderHelper).GetTypeInfo().GetDeclaredMethod(nameof(ReadColumnValue));
    ...
    public static object ReadColumnValue(IDataRecord record, int valueIndex,
